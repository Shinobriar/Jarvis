...
final class LocalDb extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE account_notification_subscriptions (" +
        ...
        db.execSQL("CREATE TABLE dm_message_reads (" +
                "message_id INTEGER NOT NULL," +
                "account_id INTEGER NOT NULL," +
                "seen_at INTEGER NOT NULL," +
                "PRIMARY KEY(message_id, account_id))");

        db.execSQL("CREATE TABLE dm_message_reactions (" +
                "message_id INTEGER NOT NULL," +
                "account_id INTEGER NOT NULL," +
        ...
        db.execSQL("CREATE TABLE dm_direct_settings (" +
        ...
        db.execSQL("CREATE TABLE dm_group_settings (" +
                "owner_id INTEGER NOT NULL," +
                "group_id INTEGER NOT NULL," +
                "muted INTEGER NOT NULL DEFAULT 0," +
                "disappearing_seconds INTEGER NOT NULL DEFAULT 0," +
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 5) {
            ...
            db.execSQL("CREATE TABLE IF NOT EXISTS dm_message_reads (" +
                    "message_id INTEGER NOT NULL," +
                    "account_id INTEGER NOT NULL," +
                    "seen_at INTEGER NOT NULL," +
                    "PRIMARY KEY(message_id, account_id))");
        ...
        if (oldVersion < 6) {
            ...
            db.execSQL("ALTER TABLE messages ADD COLUMN shared_post_id INTEGER");
            db.execSQL("CREATE TABLE IF NOT EXISTS dm_message_reactions (" +
                    "message_id INTEGER NOT NULL," +
                    "account_id INTEGER NOT NULL," +
                    "reaction TEXT NOT NULL," +
                    "created_at INTEGER NOT NULL," +
        ...
        if (oldVersion < 7) {
            db.execSQL("ALTER TABLE accounts ADD COLUMN pinned_post_id INTEGER");
            db.execSQL("ALTER TABLE posts ADD COLUMN media_paths TEXT NOT NULL DEFAULT ''");
            db.execSQL("ALTER TABLE drafts ADD COLUMN media_paths TEXT NOT NULL DEFAULT ''");
            db.execSQL("ALTER TABLE scheduled_posts ADD COLUMN media_paths TEXT NOT NULL DEFAULT ''");
            db.execSQL("UPDATE posts SET media_paths=media_path WHERE media_path IS NOT NULL AND media_path<>''");
            db.execSQL("UPDATE drafts SET media_paths=media_path WHERE media_path IS NOT NULL AND media_path<>''");
            db.execSQL("UPDATE scheduled_posts SET media_paths=media_path WHERE media_path IS NOT NULL AND media_path<>''");
        }
        if (oldVersion < 8) {
            db.execSQL("CREATE TABLE IF NOT EXISTS account_notification_subscriptions (" +
                    "subscriber_id INTEGER NOT NULL," +
                    "target_id INTEGER NOT NULL," +
                    "created_at INTEGER NOT NULL," +
                    "PRIMARY KEY(subscriber_id, target_id))");
            db.execSQL("CREATE TABLE IF NOT EXISTS dm_direct_settings (" +
                    "owner_id INTEGER NOT NULL," +
                    "peer_id INTEGER NOT NULL," +
                    "nickname TEXT NOT NULL DEFAULT ''," +
                    "muted INTEGER NOT NULL DEFAULT 0," +
                    ...
                    "security_verified INTEGER NOT NULL DEFAULT 0," +
                    "PRIMARY KEY(owner_id, peer_id))");
            db.execSQL("CREATE TABLE IF NOT EXISTS dm_group_settings (" +
                    "owner_id INTEGER NOT NULL," +
                    "group_id INTEGER NOT NULL," +
                    "muted INTEGER NOT NULL DEFAULT 0," +
                    "disappearing_seconds INTEGER NOT NULL DEFAULT 0," +
    ...
    private Account account(Cursor c) {
        ...
        int pinned = c.getColumnIndex("pinned_post_id");
        a.pinnedPostId = pinned >= 0 && !c.isNull(pinned) ? c.getLong(pinned) : null;
        return a;
    }
    ...
    long insertPost(long authorId, String body, String mediaPath, Long replyTo, Long quoteOf) {
        ...
        notifyAccountSubscribers(db, authorId, id, "post_alert");
        return id;
    }
    ...
    long insertPostWithExtras(long authorId, String body, String mediaPath, Long replyTo, Long quoteOf,
        ...
        if (mediaPath != null && !mediaPath.trim().isEmpty()) paths.add(mediaPath);
        return insertPostWithExtras(authorId, body, paths, replyTo, quoteOf, location, pollOptions);
    }

    long insertPostWithExtras(long authorId, String body, List<String> mediaPaths, Long replyTo, Long quoteOf,
                              String location, List<String> pollOptions) {
        List<String> cleanMedia = normalizeMediaPaths(mediaPaths);
        String primary = cleanMedia.isEmpty() ? null : cleanMedia.get(0);
        long id = insertPost(authorId, body, primary, replyTo, quoteOf);
        ContentValues media = new ContentValues();
        media.put("media_paths", encodeMediaPaths(cleanMedia));
        getWritableDatabase().update("posts", media, "id=?", new String[]{String.valueOf(id)});
    ...
    long schedulePost(long authorId, String body, String mediaPath, Long replyTo, Long quoteOf,
        ...
        ArrayList<String> paths = new ArrayList<>();
        if (mediaPath != null && !mediaPath.trim().isEmpty()) paths.add(mediaPath);
        return schedulePost(authorId, body, paths, replyTo, quoteOf, location, pollOptions, publishAt);
    }

    long schedulePost(long authorId, String body, List<String> mediaPaths, Long replyTo, Long quoteOf,
                      String location, List<String> pollOptions, long publishAt) {
        List<String> cleanMedia = normalizeMediaPaths(mediaPaths);
        String primary = cleanMedia.isEmpty() ? null : cleanMedia.get(0);
        ContentValues v = new ContentValues();
        ...
        if (primary == null) v.putNull("media_path"); else v.put("media_path", primary);
        v.put("media_paths", encodeMediaPaths(cleanMedia));
    ...
    int publishDueScheduled() {
        ...
        try {
            while (c.moveToNext()) {
                ...
                int mediaPathsColumn = c.getColumnIndex("media_paths");
                List<String> mediaPaths = mediaPathsColumn < 0 ? new ArrayList<>()
                        : decodeMediaPaths(c.getString(mediaPathsColumn));
                if (mediaPaths.isEmpty() && media != null && !media.trim().isEmpty()) mediaPaths.add(media);
                ...
                long postId = insertPostWithExtras(authorId, body, mediaPaths, replyTo, quoteOf, location, options);
    ...
    private List<String> normalizeMediaPaths(List<String> paths) {
        ArrayList<String> out = new ArrayList<>();
        if (paths == null) return out;
        for (String path : paths) {
            if (path == null) continue;
            String clean = path.trim().replace("\u001E", "");
            if (clean.isEmpty() || out.contains(clean)) continue;
            out.add(clean);
            if (out.size() >= 4) break;
        }
        return out;
    ...
    private String encodeMediaPaths(List<String> paths) {
        List<String> clean = normalizeMediaPaths(paths);
    ...
    private List<String> decodeMediaPaths(String encoded) {
        ArrayList<String> out = new ArrayList<>();
        if (encoded == null || encoded.trim().isEmpty()) return out;
        String[] parts = encoded.split("\u001E", -1);
        for (String part : parts) {
            if (!part.trim().isEmpty() && !out.contains(part)) out.add(part);
            if (out.size() >= 4) break;
        }
        return out;
    }
    ...
    List<Post> profileTimeline(long accountId, long viewerId) {
        ...
        Account profile = getAccount(accountId);
        if (profile != null && profile.pinnedPostId != null) {
            Post pinnedPost = null;
            for (int i = 0; i < out.size(); i++) {
                Post candidate = out.get(i);
                if (!candidate.profileRepost && candidate.id == profile.pinnedPostId) {
            ...
            if (pinnedPost == null) {
                Post candidate = getPost(profile.pinnedPostId);
                if (candidate != null && candidate.authorId == accountId) pinnedPost = candidate;
            }
            if (pinnedPost != null) {
                pinnedPost.pinned = true;
                out.add(0, pinnedPost);
            }
        }
        return out;
    }
    ...
    private Post post(Cursor c) {
        ...
        List<String> mediaPaths = mediaPathsColumn < 0 ? new ArrayList<>()
                : decodeMediaPaths(c.getString(mediaPathsColumn));
        if (mediaPaths.isEmpty() && p.mediaPath != null && !p.mediaPath.trim().isEmpty()) mediaPaths.add(p.mediaPath);
        p.mediaPaths = mediaPaths.toArray(new String[0]);
    ...
    void updatePostDirector(Post p) {
        ...
        if (p.mediaPaths != null) {
            for (String path : p.mediaPaths) mediaPaths.add(path);
        } else if (p.mediaPath != null) {
            mediaPaths.add(p.mediaPath);
        }
        v.put("media_paths", encodeMediaPaths(mediaPaths));
    ...
    boolean isPostPinned(long postId) {
        return DatabaseUtils.longForQuery(getReadableDatabase(),
                "SELECT COUNT(*) FROM accounts WHERE pinned_post_id=?",
                new String[]{String.valueOf(postId)}) > 0;
    }
    ...
    boolean pinPost(long postId) {
        ...
        value.put("pinned_post_id", postId);
        return getWritableDatabase().update("accounts", value, "id=?",
                new String[]{String.valueOf(post.authorId)}) > 0;
    }

    void unpinPost(long postId) {
        ContentValues value = new ContentValues();
        value.putNull("pinned_post_id");
        getWritableDatabase().update("accounts", value, "pinned_post_id=?",
    ...
    void deletePost(long id) {
        ...
        ContentValues clearPin = new ContentValues();
        clearPin.putNull("pinned_post_id");
        db.update("accounts", clearPin, "pinned_post_id=?", new String[]{String.valueOf(id)});
    ...
    boolean toggleInteraction(long accountId, long postId, String type) {
        ...
        if ("repost".equals(type)) notifyAccountSubscribers(db, accountId, postId, "repost_alert");
        return true;
    }
    ...
    private void notifyAccountSubscribers(SQLiteDatabase db, long actorId, long postId, String type) {
        Cursor c = db.query("account_notification_subscriptions", new String[]{"subscriber_id"},
                "target_id=? AND subscriber_id<>?", new String[]{String.valueOf(actorId), String.valueOf(actorId)},
                null, null, null);
        try {
            while (c.moveToNext()) addNotification(db, c.getLong(0), actorId, postId, type);
        } finally { c.close(); }
    }

    boolean isAccountNotificationEnabled(long subscriberId, long targetId) {
        return DatabaseUtils.longForQuery(getReadableDatabase(),
                "SELECT COUNT(*) FROM account_notification_subscriptions WHERE subscriber_id=? AND target_id=?",
                new String[]{String.valueOf(subscriberId), String.valueOf(targetId)}) > 0;
    }

    void setAccountNotificationEnabled(long subscriberId, long targetId, boolean enabled) {
        SQLiteDatabase db = getWritableDatabase();
        if (!enabled) {
            db.delete("account_notification_subscriptions", "subscriber_id=? AND target_id=?",
                    new String[]{String.valueOf(subscriberId), String.valueOf(targetId)});
            return;
        }
        ContentValues v = new ContentValues();
        v.put("subscriber_id", subscriberId);
        v.put("target_id", targetId);
        v.put("created_at", System.currentTimeMillis());
        db.insertWithOnConflict("account_notification_subscriptions", null, v, SQLiteDatabase.CONFLICT_REPLACE);
    }
    ...
    DirectConversationSettings directSettings(long ownerId, long peerId) {
        DirectConversationSettings s = new DirectConversationSettings();
        s.ownerId = ownerId;
        s.peerId = peerId;
        Cursor c = getReadableDatabase().query("dm_direct_settings", null,
                "owner_id=? AND peer_id=?", new String[]{String.valueOf(ownerId), String.valueOf(peerId)},
                null, null, null);
        try {
            if (c.moveToFirst()) {
                s.nickname = c.getString(c.getColumnIndexOrThrow("nickname"));
                s.muted = c.getInt(c.getColumnIndexOrThrow("muted")) != 0;
                s.blocked = c.getInt(c.getColumnIndexOrThrow("blocked")) != 0;
                s.disappearingSeconds = c.getLong(c.getColumnIndexOrThrow("disappearing_seconds"));
                s.blockScreenshots = c.getInt(c.getColumnIndexOrThrow("block_screenshots")) != 0;
                s.securityVerified = c.getInt(c.getColumnIndexOrThrow("security_verified")) != 0;
            }
        } finally { c.close(); }
        return s;
    }

    private void putDirectSetting(long ownerId, long peerId, String column, Object value) {
        if (!("nickname".equals(column) || "muted".equals(column) || "blocked".equals(column)
                || "disappearing_seconds".equals(column) || "block_screenshots".equals(column)
                || "security_verified".equals(column))) return;
        ContentValues base = new ContentValues();
        base.put("owner_id", ownerId);
        base.put("peer_id", peerId);
        getWritableDatabase().insertWithOnConflict("dm_direct_settings", null, base, SQLiteDatabase.CONFLICT_IGNORE);
        ContentValues v = new ContentValues();
        if (value instanceof String) v.put(column, (String) value);
        else if (value instanceof Boolean) v.put(column, (Boolean) value ? 1 : 0);
        else v.put(column, ((Number) value).longValue());
        getWritableDatabase().update("dm_direct_settings", v, "owner_id=? AND peer_id=?",
                new String[]{String.valueOf(ownerId), String.valueOf(peerId)});
    }

    void setDirectNickname(long ownerId, long peerId, String nickname) {
        putDirectSetting(ownerId, peerId, "nickname", nickname == null ? "" : nickname.trim());
    }

    void setDirectMuted(long ownerId, long peerId, boolean value) { putDirectSetting(ownerId, peerId, "muted", value); }
    void setDirectBlocked(long ownerId, long peerId, boolean value) { putDirectSetting(ownerId, peerId, "blocked", value); }
    void setDirectDisappearingSeconds(long ownerId, long peerId, long value) { putDirectSetting(ownerId, peerId, "disappearing_seconds", Math.max(0, value)); }
    void setDirectBlockScreenshots(long ownerId, long peerId, boolean value) { putDirectSetting(ownerId, peerId, "block_screenshots", value); }
    void setDirectSecurityVerified(long ownerId, long peerId, boolean value) { putDirectSetting(ownerId, peerId, "security_verified", value); }

    boolean isDirectBlocked(long a, long b) {
        return DatabaseUtils.longForQuery(getReadableDatabase(),
                "SELECT COUNT(*) FROM dm_direct_settings WHERE ((owner_id=? AND peer_id=?) OR (owner_id=? AND peer_id=?)) AND blocked=1",
                new String[]{String.valueOf(a), String.valueOf(b), String.valueOf(b), String.valueOf(a)}) > 0;
    }

    GroupConversationSettings groupSettings(long ownerId, long groupId) {
        GroupConversationSettings s = new GroupConversationSettings();
        s.ownerId = ownerId;
        s.groupId = groupId;
        Cursor c = getReadableDatabase().query("dm_group_settings", null,
                "owner_id=? AND group_id=?", new String[]{String.valueOf(ownerId), String.valueOf(groupId)},
                null, null, null);
        try {
            if (c.moveToFirst()) {
                s.muted = c.getInt(c.getColumnIndexOrThrow("muted")) != 0;
    ...
    private void putGroupSetting(long ownerId, long groupId, String column, Object value) {
        if (!("muted".equals(column) || "disappearing_seconds".equals(column) || "block_screenshots".equals(column))) return;
        ContentValues base = new ContentValues();
        base.put("owner_id", ownerId);
        base.put("group_id", groupId);
        getWritableDatabase().insertWithOnConflict("dm_group_settings", null, base, SQLiteDatabase.CONFLICT_IGNORE);
        ContentValues v = new ContentValues();
        if (value instanceof Boolean) v.put(column, (Boolean) value ? 1 : 0);
        else v.put(column, ((Number) value).longValue());
        getWritableDatabase().update("dm_group_settings", v, "owner_id=? AND group_id=?",
                new String[]{String.valueOf(ownerId), String.valueOf(groupId)});
    }

    void setGroupMuted(long ownerId, long groupId, boolean value) { putGroupSetting(ownerId, groupId, "muted", value); }
    void setGroupDisappearingSeconds(long ownerId, long groupId, long value) { putGroupSetting(ownerId, groupId, "disappearing_seconds", Math.max(0, value)); }
    void setGroupBlockScreenshots(long ownerId, long groupId, boolean value) { putGroupSetting(ownerId, groupId, "block_screenshots", value); }
    ...
    void sendMessage(long senderId, long receiverId, String body) {
        sendMessage(senderId, receiverId, body, null, null, null);
    }

    void sendMessage(long senderId, long receiverId, String body,
                     String attachmentPath, String attachmentName, String attachmentMime) {
        sendMessage(senderId, receiverId, body, attachmentPath, attachmentName, attachmentMime, null, null);
    }

    void sendMessage(long senderId, long receiverId, String body,
                     String attachmentPath, String attachmentName, String attachmentMime,
                     Long replyToMessageId, Long sharedPostId) {
        ContentValues v = new ContentValues();
        v.put("sender_id", senderId);
        v.put("receiver_id", receiverId);
        v.put("body", body == null ? "" : body);
        if (attachmentPath == null) v.putNull("attachment_path"); else v.put("attachment_path", attachmentPath);
        if (attachmentName == null) v.putNull("attachment_name"); else v.put("attachment_name", attachmentName);
        if (attachmentMime == null) v.putNull("attachment_mime"); else v.put("attachment_mime", attachmentMime);
        if (replyToMessageId == null) v.putNull("reply_to_message_id"); else v.put("reply_to_message_id", replyToMessageId);
        if (sharedPostId == null) v.putNull("shared_post_id"); else v.put("shared_post_id", sharedPostId);
        v.put("created_at", System.currentTimeMillis());
    ...
    List<DirectMessage> conversation(long a, long b) {
        ...
        long ttl = directSettings(a, b).disappearingSeconds;
        long cutoff = ttl <= 0 ? 0 : System.currentTimeMillis() - ttl * 1000L;
        Cursor c = getReadableDatabase().rawQuery(
                "SELECT * FROM messages WHERE ((sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?)) " +
                        "AND created_at>=? ORDER BY created_at ASC LIMIT 300",
                new String[]{String.valueOf(a), String.valueOf(b), String.valueOf(b), String.valueOf(a), String.valueOf(cutoff)});
        try {
    ...
    private DirectMessage message(Cursor c) {
        ...
        int api = c.getColumnIndex("attachment_path");
        int ani = c.getColumnIndex("attachment_name");
        int ami = c.getColumnIndex("attachment_mime");
        m.attachmentPath = api >= 0 && !c.isNull(api) ? c.getString(api) : null;
        m.attachmentName = ani >= 0 && !c.isNull(ani) ? c.getString(ani) : null;
        m.attachmentMime = ami >= 0 && !c.isNull(ami) ? c.getString(ami) : null;
        int rmi = c.getColumnIndex("reply_to_message_id");
        int spi = c.getColumnIndex("shared_post_id");
        m.replyToMessageId = rmi >= 0 && !c.isNull(rmi) ? c.getLong(rmi) : null;
        m.sharedPostId = spi >= 0 && !c.isNull(spi) ? c.getLong(spi) : null;
        m.createdAt = c.getLong(c.getColumnIndexOrThrow("created_at"));
        return m;
    }

    DirectMessage getMessage(long messageId) {
        Cursor c = getReadableDatabase().query("messages", null, "id=?",
                new String[]{String.valueOf(messageId)}, null, null, null);
        try { return c.moveToFirst() ? message(c) : null; }
        finally { c.close(); }
    }
    ...
    void deleteMessage(long messageId) {
        ...
        db.beginTransaction();
        try {
            db.delete("dm_message_reads", "message_id=?", new String[]{String.valueOf(messageId)});
            db.delete("dm_message_reactions", "message_id=?", new String[]{String.valueOf(messageId)});
            ContentValues detach = new ContentValues();
            detach.putNull("reply_to_message_id");
            db.update("messages", detach, "reply_to_message_id=?", new String[]{String.valueOf(messageId)});
            db.delete("messages", "id=?", new String[]{String.valueOf(messageId)});
            db.setTransactionSuccessful();
        } finally { db.endTransaction(); }
    }

    void clearDirectConversation(long a, long b) {
        SQLiteDatabase db = getWritableDatabase();
        String where = "id IN (SELECT id FROM messages WHERE (sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?))";
        String[] args = {String.valueOf(a), String.valueOf(b), String.valueOf(b), String.valueOf(a)};
        db.beginTransaction();
        try {
            db.delete("dm_message_reads", "message_id IN (SELECT id FROM messages WHERE (sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?))", args);
            db.delete("dm_message_reactions", "message_id IN (SELECT id FROM messages WHERE (sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?))", args);
            db.delete("messages", "(sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?)", args);
            db.setTransactionSuccessful();
        } finally { db.endTransaction(); }
    }

    void toggleMessageReaction(long messageId, long accountId, String reaction) {
        if (reaction == null || reaction.trim().isEmpty()) return;
        SQLiteDatabase db = getWritableDatabase();
        Cursor current = db.query("dm_message_reactions", new String[]{"reaction"},
                "message_id=? AND account_id=?",
                new String[]{String.valueOf(messageId), String.valueOf(accountId)}, null, null, null);
        String existing = null;
        try { if (current.moveToFirst()) existing = current.getString(0); }
        finally { current.close(); }
        if (reaction.equals(existing)) {
            db.delete("dm_message_reactions", "message_id=? AND account_id=?",
                    new String[]{String.valueOf(messageId), String.valueOf(accountId)});
            return;
        }
        ContentValues v = new ContentValues();
        v.put("message_id", messageId);
        v.put("account_id", accountId);
        v.put("reaction", reaction);
        v.put("created_at", System.currentTimeMillis());
        db.insertWithOnConflict("dm_message_reactions", null, v, SQLiteDatabase.CONFLICT_REPLACE);
    }

    String messageReactionForAccount(long messageId, long accountId) {
        Cursor c = getReadableDatabase().query("dm_message_reactions", new String[]{"reaction"},
                "message_id=? AND account_id=?",
                new String[]{String.valueOf(messageId), String.valueOf(accountId)}, null, null, null);
    ...
    List<MessageReactionSummary> messageReactionSummaries(long messageId, long currentAccountId) {
        ArrayList<MessageReactionSummary> out = new ArrayList<>();
        Cursor c = getReadableDatabase().rawQuery(
                "SELECT reaction, COUNT(*), MAX(CASE WHEN account_id=? THEN 1 ELSE 0 END), MIN(created_at) " +
                        "FROM dm_message_reactions WHERE message_id=? GROUP BY reaction " +
                        "ORDER BY MIN(created_at) ASC LIMIT 7",
                new String[]{String.valueOf(currentAccountId), String.valueOf(messageId)});
        try {
            while (c.moveToNext()) {
                MessageReactionSummary summary = new MessageReactionSummary();
                summary.reaction = c.getString(0);
                summary.count = c.getInt(1);
                summary.reactedByCurrent = c.getInt(2) != 0;
                out.add(summary);
            }
        } finally { c.close(); }
        return out;
    ...
    List<Account> messageReactors(long messageId, String reaction) {
        ArrayList<Account> out = new ArrayList<>();
        Cursor c = getReadableDatabase().rawQuery(
                "SELECT a.* FROM accounts a JOIN dm_message_reactions r ON r.account_id=a.id " +
                        "WHERE r.message_id=? AND r.reaction=? ORDER BY r.created_at ASC",
                new String[]{String.valueOf(messageId), reaction});
        try { while (c.moveToNext()) out.add(account(c)); }
        finally { c.close(); }
        return out;
    }

    List<Account> messageReaders(long messageId) {
        ArrayList<Account> out = new ArrayList<>();
        Cursor c = getReadableDatabase().rawQuery(
                "SELECT a.* FROM accounts a JOIN dm_message_reads r ON r.account_id=a.id " +
                        "WHERE r.message_id=? ORDER BY r.seen_at ASC",
    ...
    List<Long> messageReaderIds(long messageId) {
        ArrayList<Long> out = new ArrayList<>();
        Cursor c = getReadableDatabase().query("dm_message_reads", new String[]{"account_id"},
                "message_id=?", new String[]{String.valueOf(messageId)}, null, null, "seen_at ASC");
        try { while (c.moveToNext()) out.add(c.getLong(0)); }
        finally { c.close(); }
        return out;
    }

    void setMessageReaders(long messageId, List<Long> accountIds) {
        SQLiteDatabase db = getWritableDatabase();
        db.beginTransaction();
        try {
            db.delete("dm_message_reads", "message_id=?", new String[]{String.valueOf(messageId)});
            ...
            if (accountIds != null) {
                for (Long accountId : accountIds) {
                    ...
                    v.put("account_id", accountId);
                    v.put("seen_at", now++);
                    db.insertWithOnConflict("dm_message_reads", null, v, SQLiteDatabase.CONFLICT_IGNORE);
                }
            }
            db.setTransactionSuccessful();
        } finally { db.endTransaction(); }
    ...
    void sendGroupMessage(long senderId, long groupId, String body) {
        sendGroupMessage(senderId, groupId, body, null, null, null);
    }

    void sendGroupMessage(long senderId, long groupId, String body,
                          String attachmentPath, String attachmentName, String attachmentMime) {
        sendGroupMessage(senderId, groupId, body, attachmentPath, attachmentName, attachmentMime, null, null);
    }
    ...
    List<DirectMessage> groupConversation(long groupId) {
        return groupConversation(groupId, 0);
    }

    List<DirectMessage> groupConversation(long groupId, long ownerId) {
        ArrayList<DirectMessage> out = new ArrayList<>();
        long ttl = ownerId <= 0 ? 0 : groupSettings(ownerId, groupId).disappearingSeconds;
        long cutoff = ttl <= 0 ? 0 : System.currentTimeMillis() - ttl * 1000L;
        Cursor c = getReadableDatabase().query("messages", null, "group_id=? AND created_at>=?",
                new String[]{String.valueOf(groupId), String.valueOf(cutoff)}, null, null, "created_at ASC", "500");
        try {
            while (c.moveToNext()) out.add(message(c));
        } finally { c.close(); }
        return out;
    ...
    void clearGroupConversation(long groupId) {
        SQLiteDatabase db = getWritableDatabase();
        db.beginTransaction();
        try {
            db.delete("dm_message_reads", "message_id IN (SELECT id FROM messages WHERE group_id=?)", new String[]{String.valueOf(groupId)});
            db.delete("dm_message_reactions", "message_id IN (SELECT id FROM messages WHERE group_id=?)", new String[]{String.valueOf(groupId)});
    ...
    String lastGroupMessage(long groupId) {
        Cursor c = getReadableDatabase().rawQuery(
                "SELECT body, attachment_name, shared_post_id FROM messages WHERE group_id=? ORDER BY created_at DESC LIMIT 1",
                new String[]{String.valueOf(groupId)});
        try {
            if (!c.moveToFirst()) return "";
            String body = c.getString(0);
            if (body != null && !body.trim().isEmpty()) return body;
            String attachment = c.getString(1);
            if (attachment != null) return "Attachment · " + attachment;
            return c.isNull(2) ? "" : "Shared a post";
        }
        finally { c.close(); }
    }
    ...
    long saveDraft(long existingId, long authorId, String body, String mediaPath, Long replyTo, Long quoteOf,
        ...
        ArrayList<String> paths = new ArrayList<>();
        if (mediaPath != null && !mediaPath.trim().isEmpty()) paths.add(mediaPath);
        return saveDraft(existingId, authorId, body, paths, replyTo, quoteOf,
                location, pollOptions, scheduledAt);
    }

    long saveDraft(long existingId, long authorId, String body, List<String> mediaPaths, Long replyTo, Long quoteOf,
                   String location, List<String> pollOptions, long scheduledAt) {
        List<String> cleanMedia = normalizeMediaPaths(mediaPaths);
        String primary = cleanMedia.isEmpty() ? null : cleanMedia.get(0);
        ContentValues v = new ContentValues();
    ...
    private DraftPost draft(Cursor c) {
        ...
        List<String> mediaPaths = mediaPathsColumn < 0 ? new ArrayList<>()
                : decodeMediaPaths(c.getString(mediaPathsColumn));
        if (mediaPaths.isEmpty() && d.mediaPath != null && !d.mediaPath.trim().isEmpty()) mediaPaths.add(d.mediaPath);
        d.mediaPaths = mediaPaths.toArray(new String[0]);
        int r = c.getColumnIndexOrThrow("reply_to");
    ...
    int nonBotCount() {
        return (int) DatabaseUtils.longForQuery(getReadableDatabase(),
                "SELECT COUNT(*) FROM accounts WHERE is_bot=0", null);
    }

    int resetAllPosts() {
        SQLiteDatabase db = getWritableDatabase();
        int count = (int) DatabaseUtils.longForQuery(db, "SELECT COUNT(*) FROM posts", null);
        db.beginTransaction();
        try {
            db.execSQL("UPDATE accounts SET pinned_post_id=NULL");
            db.execSQL("UPDATE messages SET shared_post_id=NULL WHERE shared_post_id IS NOT NULL");
            db.execSQL("DELETE FROM scheduled_posts");
            db.execSQL("DELETE FROM poll_votes");
            db.execSQL("DELETE FROM drafts");
            db.execSQL("DELETE FROM notifications WHERE post_id IS NOT NULL");
            db.execSQL("DELETE FROM interactions");
            db.execSQL("DELETE FROM posts");
            db.execSQL("DELETE FROM sqlite_sequence WHERE name IN ('posts','drafts','scheduled_posts')");
            db.setTransactionSuccessful();
        } finally {
            db.endTransaction();
        }
    ...
    int deleteBotAccounts() {
        SQLiteDatabase db = getWritableDatabase();
        int count = botCount();
        if (count == 0 || nonBotCount() == 0) return 0;

        String bots = "SELECT id FROM accounts WHERE is_bot=1";
        String botPosts = "SELECT id FROM posts WHERE author_id IN (" + bots + ")";
        String botMessages = "SELECT id FROM messages WHERE sender_id IN (" + bots + ") " +
                "OR (group_id=0 AND receiver_id IN (" + bots + "))";

        db.beginTransaction();
        try {
            db.execSQL("UPDATE accounts SET pinned_post_id=NULL WHERE pinned_post_id IN (" + botPosts + ")");
            db.execSQL("UPDATE posts SET reply_to=NULL WHERE reply_to IN (" + botPosts + ")");
            db.execSQL("UPDATE posts SET quote_of=NULL WHERE quote_of IN (" + botPosts + ")");
            db.execSQL("UPDATE messages SET shared_post_id=NULL WHERE shared_post_id IN (" + botPosts + ")");
            db.execSQL("UPDATE messages SET reply_to_message_id=NULL WHERE reply_to_message_id IN (" + botMessages + ")");

            db.execSQL("DELETE FROM dm_message_reads WHERE account_id IN (" + bots + ") " +
                    "OR message_id IN (" + botMessages + ")");
            db.execSQL("DELETE FROM dm_message_reactions WHERE account_id IN (" + bots + ") " +
                    "OR message_id IN (" + botMessages + ")");
            db.execSQL("DELETE FROM messages WHERE id IN (" + botMessages + ")");

            db.execSQL("DELETE FROM notifications WHERE account_id IN (" + bots + ") " +
                    "OR actor_id IN (" + bots + ") OR post_id IN (" + botPosts + ")");
            db.execSQL("DELETE FROM interactions WHERE account_id IN (" + bots + ") " +
                    "OR post_id IN (" + botPosts + ")");
            db.execSQL("DELETE FROM poll_votes WHERE account_id IN (" + bots + ") " +
                    "OR post_id IN (" + botPosts + ")");
            db.execSQL("DELETE FROM drafts WHERE author_id IN (" + bots + ")");
            db.execSQL("DELETE FROM scheduled_posts WHERE author_id IN (" + bots + ")");
            db.execSQL("DELETE FROM posts WHERE author_id IN (" + bots + ")");
            db.execSQL("DELETE FROM follows WHERE follower_id IN (" + bots + ") " +
                    "OR following_id IN (" + bots + ")");

            db.execSQL("DELETE FROM dm_group_members WHERE account_id IN (" + bots + ")");
            db.execSQL("UPDATE dm_groups SET created_by=(SELECT MIN(account_id) FROM dm_group_members " +
                    "WHERE group_id=dm_groups.id) WHERE created_by IN (" + bots + ") " +
                    "AND EXISTS (SELECT 1 FROM dm_group_members WHERE group_id=dm_groups.id)");
            String emptyGroups = "SELECT id FROM dm_groups WHERE NOT EXISTS " +
                    "(SELECT 1 FROM dm_group_members WHERE group_id=dm_groups.id)";
            db.execSQL("DELETE FROM dm_message_reads WHERE message_id IN " +
                    "(SELECT id FROM messages WHERE group_id IN (" + emptyGroups + "))");
            db.execSQL("DELETE FROM dm_message_reactions WHERE message_id IN " +
                    "(SELECT id FROM messages WHERE group_id IN (" + emptyGroups + "))");
            db.execSQL("DELETE FROM messages WHERE group_id IN (" + emptyGroups + ")");
            db.execSQL("DELETE FROM dm_groups WHERE id IN (" + emptyGroups + ")");

            db.execSQL("DELETE FROM accounts WHERE is_bot=1");
            db.execSQL("UPDATE posts SET replies=(SELECT COUNT(*) FROM posts child WHERE child.reply_to=posts.id)");
            db.setTransactionSuccessful();
        } finally {
            db.endTransaction();
        }
