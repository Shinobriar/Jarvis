...
public class MainActivity extends Activity {
    ...
    private LinearLayout xLogoTopBar() {
        ...
        XUi.IconView plus = new XUi.IconView(this, XUi.IconView.PLUS, pal.fg);
        plus.setLayoutParams(new LinearLayout.LayoutParams(dp(40), dp(40)));
        plus.setPadding(dp(11), dp(11), dp(11), dp(11));
        plus.setOnClickListener(v -> showCreateAccount());
        bar.addView(plus);
    ...
    private void addNavIcon(LinearLayout nav, int type, boolean active, Runnable action) {
        ...
        iv.setActive(active);
        FrameLayout.LayoutParams ip = new FrameLayout.LayoutParams(dp(29), dp(29), Gravity.CENTER);
    ...
    private void addComposeFab(FrameLayout frame) {
        ...
        XUi.IconView plus = new XUi.IconView(this, XUi.IconView.PLUS, Color.WHITE);
        FrameLayout.LayoutParams pp = new FrameLayout.LayoutParams(dp(28), dp(28), Gravity.CENTER);
    ...
    private View postView(Post p, boolean detail) {
        ...
        if (a.verified) {
            TextView check = tv(" ✓", 15, XUi.BLUE, true);
            meta.addView(check);
        }
    ...
    private View quotedPost(Post q) {
        ...
        if (qa != null) {
            LinearLayout meta = hbox();
            TextView n = tv(qa.name + (qa.verified ? " ✓" : ""), 14, qa.verified ? XUi.BLUE : pal.fg, true);
            meta.addView(n);
    ...
    private void renderProfile(long accountId) {
        ...
        if (a.verified) nameRow.addView(tv(" ✓", 18, XUi.BLUE, true));
        body.addView(info);
    ...
    private View accountRow(Account a) {
        ...
        if (a.verified) nrow.addView(tv(" ✓", 14, XUi.BLUE, true));
        labels.addView(nrow);
    ...
    private void showComposer(Long replyTo, Long quoteOf) {
        ...
        labels.addView(tv(author.name + (author.verified ? " ✓" : ""), 15, author.verified ? XUi.BLUE : pal.fg, true));
    ...
    private void showAccountSwitcher() {
        ...
        for (Account a : db.listAccounts()) {
            ...
            LinearLayout labels = vbox();
            labels.setLayoutParams(new LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f));
            labels.addView(tv(a.name + (a.verified ? " ✓" : ""), 15, a.verified ? XUi.BLUE : pal.fg, true));
            labels.addView(tv("@" + a.handle, 14, pal.secondary, false));
            row.addView(labels);
            if (a.id == currentAccountId) row.addView(tv("✓", 20, XUi.BLUE, true));
            row.setOnClickListener(v -> {
