...
final class XProfileImporter {
    ...
    static final class Result {
        ...
        String avatarPath;
        String bannerPath;
        String unclassifiedImageUrl;
        boolean pagePreviewWasBanner;
        long createdAt;
        long followers = -1;
        long following = -1;
        boolean verified;
    ...
    static Result importProfile(Context context, String handle, String bearerToken) throws Exception {
        ...
        if (result.avatarUrl != null && !result.avatarUrl.trim().isEmpty()) {
            try {
                ...
                if (isWideImage(result.avatarPath)) {
                    result.bannerPath = result.avatarPath;
                    result.avatarPath = null;
                    result.pagePreviewWasBanner = true;
                }
            } catch (Exception e) {
                result.warning = appendWarning(result.warning, "The profile picture could not be downloaded.");
            }
        }
        if (result.bannerPath == null && result.bannerUrl != null && !result.bannerUrl.trim().isEmpty()) {
            try {
        ...
        if (result.unclassifiedImageUrl != null && !result.unclassifiedImageUrl.trim().isEmpty()) {
            try {
                String path = downloadImage(context, result.unclassifiedImageUrl, "profile-preview");
                if (isWideImage(path)) {
                    if (result.bannerPath == null) result.bannerPath = path;
                    else new File(path).delete();
                    result.pagePreviewWasBanner = true;
                } else {
                    if (result.avatarPath == null) result.avatarPath = path;
                    else new File(path).delete();
                }
            } catch (Exception e) {
                result.warning = appendWarning(result.warning, "X's profile preview image could not be downloaded.");
            }
        }
        return result;
    }
    ...
    private static Result fromPublicPage(String handle) throws Exception {
        ...
        result.avatarUrl = jsonStringValue(html, "profile_image_url_https");
        if (result.avatarUrl.isEmpty()) result.avatarUrl = jsonStringValue(html, "profile_image_url");
        result.bannerUrl = jsonStringValue(html, "profile_banner_url");

        if (!image.isEmpty()) {
            String imagePath = image.toLowerCase(Locale.US);
            if (imagePath.contains("/profile_images/") && result.avatarUrl.isEmpty()) {
                result.avatarUrl = image;
            } else if (imagePath.contains("/profile_banners/") && result.bannerUrl.isEmpty()) {
                result.bannerUrl = image;
                result.pagePreviewWasBanner = true;
            } else if (!image.equals(result.avatarUrl) && !image.equals(result.bannerUrl)
                    && (result.avatarUrl.isEmpty() || result.bannerUrl.isEmpty())) {
                result.unclassifiedImageUrl = image;
            }
        }
        if (result.avatarUrl.isEmpty()) result.avatarUrl = null;
        if (result.bannerUrl.isEmpty()) result.bannerUrl = null;
        result.partial = true;
    ...
    private static String jsonStringValue(String input, String key) {
        if (input == null || input.isEmpty()) return "";
        Matcher matcher = Pattern.compile("\\\"" + Pattern.quote(key)
                + "\\\"\\s*:\\s*\\\"((?:\\\\.|[^\\\"])*)\\\"").matcher(input);
        if (!matcher.find()) return "";
        try {
            return new JSONArray("[\"" + matcher.group(1) + "\"]").getString(0);
        } catch (Exception ignored) {
    ...
    private static boolean isWideImage(String path) throws Exception {
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeFile(path, options);
        if (options.outWidth <= 0 || options.outHeight <= 0) {
            throw new Exception("Downloaded image could not be identified");
        }
        return options.outWidth >= options.outHeight * 1.35f;
    }
